...
import {
  ...
  ChevronDown, PackageSearch, TrendingUp,
} from "lucide-react";
  ...
  const fetchProducts = useCallback(async (p = 1) => {
    ...
    try {
      ...
      if (priceRange.max !== Infinity || priceRange.min > 0) {
        list = list.filter((prod) => prod.price >= priceRange.min && prod.price <= priceRange.max);
      }
    ...
    <>
      ...
      <main className="w-full px-4 sm:px-6 xl:px-10 py-6 min-h-screen">
        ...
        <div className="mb-8 flex items-start gap-4">
          ...
          <div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-900 tracking-tight">Trending Now</h1>
            <p className="text-sm text-gray-400 mt-1">
              {loading
                ? "Loading…"
                : <><span className="text-gray-700 font-semibold">{total}</span> trending products</>}
            </p>
          </div>
        </div>
          ...
          <div className="min-w-0 flex flex-col">
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center gap-5 text-center pt-20">
                ...
                <div>
                  <p className="text-xl font-extrabold text-gray-900 mb-1.5">No trending products found</p>
                ...
                {hasFilters && (
                  <button onClick={clearAll}
                    className="px-6 py-2.5 bg-violet-600 hover:bg-violet-700 text-white text-sm font-semibold rounded-full transition-colors">
                    Clear filters
                  </button>
                )}
              </div>
                ...
                {pages > 1 && (
                  <div className="flex items-center justify-center gap-1.5 mt-10">
                    <button onClick={() => fetchProducts(page - 1)} disabled={page === 1}
                      className="w-9 h-9 flex items-center justify-center rounded-xl border border-gray-200 text-gray-600 hover:bg-gray-50 disabled:opacity-30 transition">
                      ...
                      n === "…" ? (
                        ...
                        <button key={n} onClick={() => fetchProducts(n)}
                          className={`w-9 h-9 flex items-center justify-center rounded-xl text-sm font-semibold transition ${
                            page === n ? "bg-violet-600 text-white shadow-sm" : "border border-gray-200 text-gray-600 hover:bg-gray-50"
                          }`}>
                          {n}
                        </button>
                      )
