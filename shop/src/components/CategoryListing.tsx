...
import {
  Search, X, SlidersHorizontal, ChevronRight, ChevronDown,
  PackageSearch, ChevronLeft,
} from "lucide-react";
...
interface SidebarProps {
  search: string; priceLabel: string; sort: string; hasFilters: boolean;
  setSearch: (v: string) => void; setPriceLabel: (v: string) => void;
  setSort: (v: string) => void; clearAll: () => void;
}

function SidebarFilters({ search, priceLabel, sort, hasFilters, setSearch, setPriceLabel, setSort, clearAll }: SidebarProps) {
  return (
    <div>
...
interface Props {
  category: string;       // "Cases", "Screen Protection", etc.
  categorySlug: string;   // "cases", "screen-protection", etc.
}
...
export default function CategoryListing({ category, categorySlug }: Props) {
  ...
  const hasFilters = priceLabel !== "All Prices" || sort !== "newest" || search.trim() !== "";
  const activeCount = [priceLabel !== "All Prices", sort !== "newest", search.trim() !== ""].filter(Boolean).length;
  ...
  return (
    <>
      ...
      <main className="w-full px-4 sm:px-6 xl:px-10 py-6 min-h-screen">
        ...
        {/* Category header */}
        <div className="mb-8">
          <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-900 tracking-tight">{category}</h1>
          <p className="text-sm text-gray-400 mt-1">
            {loading
              ? "Loading…"
              : <><span className="text-gray-700 font-semibold">{total}</span> products</>}
          </p>
        </div>
