...
export default function CartSidebar({ open, onClose }: Props) {
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className={`fixed inset-0 z-[150] bg-black/40 transition-opacity duration-300 ${visible ? "opacity-100" : "opacity-0"}`}
        onClick={onClose}
      />

      {/* Drawer */}
      <div className={`fixed top-0 right-0 bottom-0 z-[160] w-full max-w-sm bg-white shadow-2xl flex flex-col transition-transform duration-300 ease-out ${visible ? "translate-x-0" : "translate-x-full"}`}>
        ...
        <div className="flex-1 overflow-y-auto">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-4 px-8 text-center">
              ...
              <button onClick={onClose} className="mt-2 px-6 py-2.5 bg-violet-600 hover:bg-violet-700 text-white text-sm font-semibold rounded-full transition-colors">
                Continue Shopping
              </button>
            </div>
          ) : (
            <ul className="divide-y divide-gray-100">
              {items.map((item) => (
                <li key={`${item.id}-${item.color}`} className="flex gap-3 px-5 py-4">
                  {/* Image */}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm font-semibold text-gray-900 leading-snug line-clamp-2 mt-0.5">{item.name}</p>
                    {item.color && (
                    ...
                    <div className="flex items-center justify-between mt-2 gap-2">
                      {/* Qty controls */}
                      <div className="flex items-center gap-1.5 bg-gray-50 border border-gray-200 rounded-xl px-1 py-0.5">
                        <button
                          onClick={() => updateQuantity(item.id, item.color, item.quantity - 1)}
                          className="w-6 h-6 flex items-center justify-center rounded-lg text-gray-500 hover:bg-gray-200 transition"
                        >
        ...
        {items.length > 0 && (
          <div className="border-t border-gray-100 px-5 py-5 space-y-3 bg-gray-50/60">
            <div className="flex items-center justify-between">
              ...
              <span className="text-lg font-extrabold text-gray-900">PKR {totalPrice().toLocaleString()}</span>
            </div>
            ...
            <button onClick={onClose} className="w-full py-2.5 text-sm font-semibold text-gray-600 hover:text-gray-900 transition-colors">
              Continue Shopping
            </button>
          </div>
        )}
