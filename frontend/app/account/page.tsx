...
import { User, Lock, ShoppingBag, Calendar, LogOut, ChevronRight, Loader2 } from "lucide-react";
...
function ProfileSection({ user, onUpdate }: { user: { name: string; email: string }; onUpdate: (name: string) => void }) {
        ...
        <div className="flex justify-end">
          <button
            ...
            className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all disabled:opacity-60 shadow-sm ${
              saved ? "bg-emerald-500 text-white" : "bg-violet-600 hover:bg-violet-700 text-white shadow-violet-200"
            }`}
          >
...
const QUICK_LINKS = [
  { href: "/appointment", icon: Calendar,    label: "Book Repair",   sub: "Schedule a device repair" },
  { href: "http://localhost:3001", icon: ShoppingBag, label: "MMZ Shop", sub: "Shop accessories & parts", external: true },
];
...
export default function AccountPage() {
  ...
  if (!initialized) {
    return (
      <>
        <Header />
        <main className="flex-1 bg-gray-50 min-h-screen pt-20 sm:pt-24">
  ...
  if (!user) {
    return (
      <>
        <Header />
        <main className="flex-1 bg-gray-50 min-h-screen pt-20 sm:pt-24 flex flex-col items-center justify-center text-center px-4">
  ...
  return (
    <>
      <Header />
      <main className="flex-1 bg-gray-50 min-h-screen pt-20 sm:pt-24 pb-32 sm:pb-10">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          {QUICK_LINKS.map(({ href, icon: Icon, label, sub, external }) => (
            <Link
              key={href}
              href={href}
        ...
        <div className="space-y-4">
          <ProfileSection user={user} onUpdate={handleNameUpdate} />
          <PasswordSection />
